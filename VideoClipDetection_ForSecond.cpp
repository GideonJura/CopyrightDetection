...
const int QUERY_SAMPLE_NUMBER = 10; //desperated
const int DOWNSAMPLE_FRAME_PER_SECOND = 2;
...
void PersistOnDisk(VideoCapture capture, string file_name)
{
	...
	while (cur_index < total_frame * DOWNSAMPLE_FRAME_PER_SECOND) {

		//Persist DOWNSAMPLE_FRAME_PER_SECOND frames per second on the database
		capture.set( CV_CAP_PROP_POS_FRAMES,cur_index / DOWNSAMPLE_FRAME_PER_SECOND); 
...
int CompareMatWithHistory(Mat query_frame, string file_name, int reference_start_block, int reference_end_block, int reference_fps, int& max_similar_rate)
{
	clock_t start, finish;  
	double duration;  
	long i = 10000000L; 
	...
	Mat reference_descriptor;
	int best_matched_img_index = 0;
	int max_match_number = 0;

	for(int cur_index = reference_start_block ; cur_index <= reference_end_block; cur_index ++)
	{
		stringstream ss;
		ss<<cur_index;	
		string absolute_file_name = "F://" + file_name + "//" + ss.str();
		...
		if(similar_size > max_match_number)
		{
			...
			best_matched_img_index = cur_index;
		}
	}

	max_similar_rate = max_match_number;
	int best_matched_img_frame = best_matched_img_index * reference_fps / DOWNSAMPLE_FRAME_PER_SECOND;

	finish = clock(); 
	duration = (double)(finish - start) / CLOCKS_PER_SEC;  
	printf( "CompareMatWithHistory : %f seconds\n", duration ); 

	return best_matched_img_frame;
}
...
int CompareMovieWithMovie(char* reference_movie, char* query_movie, char* movie_folder_name)
{
	...
	int reference_total_block = reference_total_frame * DOWNSAMPLE_FRAME_PER_SECOND / reference_cap.get(CV_CAP_PROP_FPS);
	int query_fps = query_cap.get(CV_CAP_PROP_FPS);
	int reference_fps = reference_cap.get(CV_CAP_PROP_FPS);

	Mat query_frame;
	
	long query_block_left_border[QUERY_SAMPLE_NUMBER];
	long query_block_right_border[QUERY_SAMPLE_NUMBER];
	bool is_frame_in_this_clip[QUERY_SAMPLE_NUMBER];
	long best_frame_index[QUERY_SAMPLE_NUMBER];
	long reference_start_frame_index[QUERY_SAMPLE_NUMBER];
	long query_start_frame_index[QUERY_SAMPLE_NUMBER];


	int reference_begin_frame = 0;
	int good_match_number = 0;
	int longest_block_index = -1;
	int longest_block_length = 0; 

	//for(int i = 5 ; i < 6 ; i++)
	for(int i = 0 ; i < QUERY_SAMPLE_NUMBER ; i++)
	{		
		cout<<"GO TO NUM: "<<i<<" Query Frames(0 - 9)"<<endl;
		//choose the middle frame for example
		long query_begin_frame = query_total_frame / QUERY_SAMPLE_NUMBER * i  + i;
		...
		reference_begin_frame = CompareMatWithHistory(croped_frame, movie_folder_name, 0 , reference_total_block, reference_cap.get(CV_CAP_PROP_FPS), good_match_number);
		...
		{
			cout<<"Found @"<<reference_begin_frame<<"--Matches:"<<good_match_number<<endl;
			is_frame_in_this_clip[i] = true;
		}

		int off_size = AlignmentFrameWithinMovie(query_frame, reference_movie, reference_begin_frame, QUERY_SAMPLE_NUMBER);
		cout<<"Alignment_Off_Size: "<<off_size<<endl;

		//query_begin_frame 1464 total frame 2928 raw_frame 3940 off_size 1 block 20
		int query_clip_begin_block = BinarySearchBorderLeft(query_cap,reference_cap, query_begin_frame - off_size, query_total_frame, reference_begin_frame, query_fps, reference_fps, movie_folder_name);

		//query_begin_frame 1464 total frame 2928 raw_frame 3940 off_size = 1
		int query_clip_end_block = BinarySearchBorderRight(query_cap,reference_cap, query_begin_frame - off_size, query_total_frame, reference_begin_frame, query_fps, reference_fps, movie_folder_name);

		int blocks = query_clip_end_block - query_clip_begin_block;
		//query_start_frame_index[i] = (query_begin_frame - off_size) % QUERY_SAMPLE_NUMBER + QUERY_SAMPLE_NUMBER * query_clip_begin_block;
		//reference_start_frame_index[i] = reference_begin_frame - (query_begin_frame - off_size - query_start_frame_index[i]);

		//Don't forget to add the offset
		query_start_frame_index[i] = (query_begin_frame - off_size) % QUERY_SAMPLE_NUMBER + query_fps * query_clip_begin_block / DOWNSAMPLE_FRAME_PER_SECOND;
		//TODO could less than 0
		reference_start_frame_index[i] = reference_begin_frame - (query_begin_frame - off_size - query_start_frame_index[i]) * reference_fps / query_fps ;
	...
	for(int j = 0 ; j < QUERY_SAMPLE_NUMBER ; j++)
	{
		...
		cout<<"Reference Start"<<reference_start_frame_index[j]<<" Query Start "<< query_start_frame_index[j]<<endl;
		cout<<"Longest Block: "<<longest_block_index<<endl;
	}
	...
	if(longest_block_index  >= 0)
	{
		...
		int reference_start_max_frame = reference_start_frame_index[longest_block_index];
		int query_start_max_frame = query_start_frame_index[longest_block_index];
		...
		for(int k = 0 ; k < QUERY_SAMPLE_NUMBER ; k++)
		{
			...
			{
				//Validate
				if( abs((query_block_left_border[k] - query_begin_max_block) - (query_start_frame_index[k] - query_start_max_frame) / QUERY_SAMPLE_NUMBER) < merge_clip_max_offset_threshold)
				{
					reference_start_max_frame = reference_start_frame_index[k];
					query_start_max_frame = query_start_frame_index[k];
		...
		cout<<"#FINAL RESULT"<<":Begin Time:"<<query_begin_max_block / DOWNSAMPLE_FRAME_PER_SECOND / 60 <<"MIN"<<(query_begin_max_block / DOWNSAMPLE_FRAME_PER_SECOND) % 60 <<"SECOND"<<endl;
		cout<<"#FINAL RESULT"<<":End Time:"<<query_end_max_block / DOWNSAMPLE_FRAME_PER_SECOND / 60 <<"MIN"<<(query_end_max_block / DOWNSAMPLE_FRAME_PER_SECOND) % 60 <<"SECOND"<<endl;
		cout<<"#FINAL RESULT"<<":Total Time:"<<(query_end_max_block - query_begin_max_block) / DOWNSAMPLE_FRAME_PER_SECOND<<"SECOND"<<endl;
		cout<<"#######################################"<<endl;
...
void PlayTwoMovie(VideoCapture query_cap, VideoCapture reference_cap, int start_query, int start_reference, int blocks )
{
	...
	{
		int frame_in_the_reference = start_reference + i * reference_fps / DOWNSAMPLE_FRAME_PER_SECOND;
		int frame_in_the_query = start_query + i * query_fps / DOWNSAMPLE_FRAME_PER_SECOND;
...
{
	int query_begin_block = aligned_query_begin_frame * DOWNSAMPLE_FRAME_PER_SECOND / query_fps;
	int reference_start = raw_reference_frame - query_begin_block * reference_fps / DOWNSAMPLE_FRAME_PER_SECOND;
	int alignment_query_start = aligned_query_begin_frame - query_begin_block * query_fps / DOWNSAMPLE_FRAME_PER_SECOND;
	//find the first block match the refernece
	int start_block = 0;
	int end_block = query_begin_block;
	...
	while(end_block > start_block)
	{
		...
		int frame_in_the_reference = reference_start + cur_block * reference_fps / DOWNSAMPLE_FRAME_PER_SECOND;
		int frame_in_the_query = alignment_query_start + cur_block * query_fps / DOWNSAMPLE_FRAME_PER_SECOND;
...
int BinarySearchBorderRight(VideoCapture query_cap, VideoCapture reference_cap,int aligned_query_begin_frame, int total_query_frame, int raw_reference_frame, int query_fps, int reference_fps, char* movie_folder_name)
{
	...
	int alignment_query_start = aligned_query_begin_frame - query_begin_block * query_fps / DOWNSAMPLE_FRAME_PER_SECOND;
	//find the first block match the refernece
	int start_block = query_begin_block;
	int end_block = total_query_frame * DOWNSAMPLE_FRAME_PER_SECOND / query_fps;
...
int AlignmentFrameWithinMovie(Mat query_mat, char* movie_path, int reference_begin_frame, int downsample_rate)
{
	...
	long start_frame = (reference_begin_frame - downsample_rate) >= 0 ? (reference_begin_frame - downsample_rate) : 0;
	long end_frame   = (reference_begin_frame + downsample_rate) <= 1000000 ? (reference_begin_frame + downsample_rate) : CV_CAP_PROP_FRAME_COUNT;
	...
	for(long curFrame = start_frame + 1; curFrame< end_frame; curFrame ++)
	{
		...
		if(good_match_proportion > best_match_proprotion)
		{
			...
			offsize = curFrame - reference_begin_frame;
		}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//preprocessing(reference_clip,reference_folder_name);
	CompareMovieWithMovie(reference_clip, query_clip4,reference_folder_name);
	...
	char* reference_clip_test = "F:\\core_dataset\\core_dataset\\beautiful_mind_game_theory\\46f2e964ae16f5c27fad70d6849c76616fad7502.mp4";
	char* query_clip_now_ok =   "F:\\core_dataset\\core_dataset\\beautiful_mind_game_theory\\904c8ebf782357ae78ebd205fe3428ad76b975a5.mp4";
	preprocessing(reference_clip_test,reference_folder_name);
	CompareMovieWithMovie(reference_clip_test, query_clip_now_ok,reference_folder_name);
	*/
